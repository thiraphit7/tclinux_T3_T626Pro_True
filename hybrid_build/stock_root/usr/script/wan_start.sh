...
if [ "$TCSUPPORT_SDN_OVS" != "" ] || [ "$TCSUPPORT_CT_VPN_PPTP" != "" ]; then
PPP_PLUGIN=libpppoe.so
else
PPP_PLUGIN=pppoe
fi
...
if [ "$TCSUPPORT_CT_VLAN_TAG" = "" ] ;then		
	...
	if [ "$IPVERSION" = "IPv4/IPv6" ]|| [ "$IPVERSION" = "IPv6" ]; then
		ip6tables -A FORWARD -o $WAN_IF -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
	fi
	else
...
#if/*TCSUPPORT_COMPILE*/ defined(TCSUPPORT_CT_PON)
	if [ "$TCSUPPORT_CT_PON" != "" ]; then
		PPP_PARAM="$PPP_PARAM plugin $PPP_PLUGIN  $NASName"
...
#	echo "pppd $PPP_PARAM &" > /var/tmp/$PPP_IF.conf
	...
	if [ "$IPVERSION" = "IPv4/IPv6" ]|| [ "$IPVERSION" = "IPv6" ]; then
		ip6tables -D FORWARD -o $PPP_IF -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $LASTmss
	fi
	fi
	...
	if [ "$IPVERSION" = "IPv4/IPv6" ]|| [ "$IPVERSION" = "IPv6" ]; then
		ip6tables -A FORWARD -o $PPP_IF -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
	fi
	else
		iptables -A FORWARD -o $PPP_IF -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $MSS
	if [ "$IPVERSION" = "IPv4/IPv6" ]|| [ "$IPVERSION" = "IPv6" ]; then
		ip6tables -A FORWARD -o $PPP_IF -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss $MSS
	fi
	fi
	if [ "$NATENABLE" = "Enable" ] ; then
#		iptables -t nat -A POSTROUTING -j ADDRMAP_POS$i
