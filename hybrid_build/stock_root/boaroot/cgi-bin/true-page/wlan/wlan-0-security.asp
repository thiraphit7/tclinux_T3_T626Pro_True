...
if Request_Form("wlanWEPFlag")="1" Then
  if Request_Form("WEP_Selection") = "WEP" then
    TCWebApi_set("WLan11ac_Entry","AuthMode","WEP_Selection2")
    TCWebApi_set("WLan11ac_Entry","WEPAuthType","authType")
    tcWebApi_constSet("WLan11ac_Entry","DefaultKeyID","1")
    TCWebApi_set("WLan11ac_Entry","Key1Str","WEP_Key")
  else
    ...
    TCWebApi_set("WLan11ac_Entry","RekeyInterval","WPARekeyInter")
  end if
...
<body>
  ...
  <div class="container-full container-resize-height">
    <form name=ConfigForm action="/cgi-bin/true-page/wlan/wlan-0-security.asp" method="post" >
...
<%if 1 = 1 then%>
            <option value="5" <% if TCWebApi_get("WebCurSet_Entry","wlan_ac_id","h") = "5" then asp_write("selected") end if %> >SSID5 ( <% TCWebApi_get("WLan11ac_Entry5","SSID","s") %> )</option>
            <option value="6" <% if TCWebApi_get("WebCurSet_Entry","wlan_ac_id","h") = "6" then asp_write("selected") end if %> >SSID6 ( <% TCWebApi_get("WLan11ac_Entry6","SSID","s") %> )</option>
            <option value="7" <% if TCWebApi_get("WebCurSet_Entry","wlan_ac_id","h") = "7" then asp_write("selected") end if %> >SSID7 ( <% TCWebApi_get("WLan11ac_Entry7","SSID","s") %> )</option>
<% end if %>
          </select>
        </div>
      </div>
      <div class="row p-1">
        ...
        <div class="col-6">
          <select name="WEP_Selection" onChange="doWEPChange()" class="form-control input-select">
            ...
            <option value="WEP" <% if tcWebApi_get("WLan11ac_Entry","AuthMode","h") = "WEP-64Bits" then asp_Write("selected") end if if tcWebApi_get("WLan11ac_Entry","AuthMode","h") = "WEP-128Bits" then asp_Write("selected") end if %>>WEP</option>
      ...
      <div id="upTime" class="row p-1">
        ...
        <div class="col-6"> 
          <input class="form-control input-textfield" type="text" id="WPARekeyInter" NAME="WPARekeyInter" SIZE="7" MAXLENGTH="7" onBlur="checkRekeyinteral(this.value, 0)" value="<% If tcWebApi_get("WLan11ac_Entry","RekeyInterval","h") <> "N/A" then tcWebApi_get("WLan11ac_Entry","RekeyInterval","s") end if %>" >
        </div>
      </div>
      <div id="WPA_div" class="element-hide">
        ...
        <div class="row p-1">
          ...
          <div class="col-4">
            ...
            value="<% If tcWebApi_get("WLan11ac_Entry","WPAPSK","h") <> "N/A" then tcWebApi_get("WLan11ac_Entry","WPAPSK","s") end if %>">
          </div>
      ...
      <div id="WEP_div" class="element-hide">
        <div class="row p-1">
          ...
          <div class="col-6">
            <INPUT type=radio value="OpenSystem" name="authType" <%if tcWebApi_get("WLan11ac_Entry","WEPAuthType","h") = "OpenSystem" then asp_Write("checked") end if %>>
            <label class="input-radio">Open System</label>
            <INPUT type=radio value="SharedKey" name="authType" <%if tcWebApi_get("WLan11ac_Entry","WEPAuthType","h") = "SharedKey" then asp_Write("checked") end if %>> 
            <label class="input-radio">Shared Key</label>
            <INPUT type=radio value="WEPAuto" name="authType" <%if tcWebApi_get("WLan11ac_Entry","WEPAuthType","h") = "WEPAuto" then asp_Write("checked") elseif tcWebApi_get("WLan11ac_Entry","WEPAuthType","h") = "N/A" then asp_Write("checked") end if %>> 
        ...
        <div class="row p-1">
          ...
          <div class="col-6">
            <select name="WEP_Selection2" onChange="doWEPChange()" class="form-control input-select">
              <option value="WEP-64Bits" <% if tcWebApi_get("WLan11ac_Entry","AuthMode","h") = "WEP-64Bits" then asp_Write("selected") end if %>>WEP-64 Bits</option>
              <option value="WEP-128Bits" <% if tcWebApi_get("WLan11ac_Entry","AuthMode","h") = "WEP-128Bits" then asp_Write("selected") end if %>>WEP-128 Bits</option>
            </select>
          </div>
        </div>
        ...
        <div class="row p-1">
          ...
          <div class="col-6">
            <input class="form-control input-textfield" type="text" name="WEP_Key" maxlength="26" value="<% If tcWebApi_get("WLan11ac_Entry","Key1Str","h") <> "N/A" then tcWebApi_get("WLan11ac_Entry","Key1Str","s") end if %>" >
          </div>
        </div>
        <div class="row p-1">
          <div class="col-6">
  ...
  <script>
    ...
    function doEncryptionChange(object){
      ...
      <% if tcWebApi_get("WLan11ac_Entry1","WscV2Support","h") <> "1"  then %>
        <% If tcWebApi_get("WLan11ac_Entry","WPSConfMode","h") = "1" then %>
          if((WEPSelectIndex == 1)
          {
