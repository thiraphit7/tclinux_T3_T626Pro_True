...
<% end if %>
    ...
    <script language=JavaScript type=text/javascript>
    var wlanSelfPinCode_flag = "<% tcWebApi_get("Info_WLan", "wlanSelfPinCode", "s") %>";
    ...
    {
        var vStartSSID = "<% tcWebApi_get("WLan_Entry","StartSSID","s") %>";
  ...
  {
      var vStartSSID = "<% tcWebApi_get("WLan_Entry","StartSSID","s") %>";
      var vEntry = "<% tcWebApi_get("WLan_Entry","AuthMode","s") %>";
    ...
    function load()
    {
        var wpsenable = "<% tcWebApi_get("WLan_Entry","WPSConfMode","s") %>";
...
<body onload="load()">
<!-- content -->
  <form name=ConfigForm method="post" action="/cgi-bin/true-page/wlan/wlan-1-wps.asp">
    ...
    <input type="hidden" name="isInWPSing" value="<%tcWebApi_get("Info_WLan","wlanWPStimerRunning_1","s")%>">
    <% else %>
    ...
    <div class="container-full container-resize-height">
      ...
      <div class="row p-1">
        <div class="col-6 pl60">
          ...
          <% if tcWebApi_get("WLan_Entry","WPSConfMode","h") = "0" then asp_Write("checked") end if%> onclick="">
          <label class="input-radio">Disable WPS</label>
          <label class="input-radio" id='tips' style="display: none">
            <a href="/cgi-bin/true-page/wlan/wlan-1-basic.asp">
              <strong><font color='#FF0000' size='-1'>Please enable 2.4g wifi</font></strong>
            </a>
          </label>
        </div>
      </div>
      ...
      <div class="card-table-content">
        ...
        <table style="width:100%">
          ...
          <tr class="table-content-fist center">
            <td><% tcWebApi_get("WLan_Entry","SSID","s") %></td>
            <script language="JavaScript" type="text/JavaScript">
            ...
            else if("WEP" == Auth)
            {
              ...
              var WEPkey = "<%if tcWebApi_get("WLan_Entry","Key1Str","h") <> "N/A" then tcWebApi_get("WLan_Entry","Key1Str","s") end if%>";
              document.writeln('<td>' + WEPkey + '</td>');
            }
            else
            {
              var encryptype = "<% tcWebApi_get("WLan_Entry","EncrypType","s") %>";
              if("AES" == encryptype)
              ...
              var WPAPSK = "<%if tcWebApi_get("WLan_Entry","WPAPSK","h") <> "N/A" then tcWebApi_get("WLan_Entry","WPAPSK","s") end if%>";
              document.writeln('<td>' + WPAPSK + '</td>');
            }
            </script>
            <td><%if tcWebApi_get("WLan_Entry","StartSSID","h") <> "1" then %>
                  No
                <%else%>
                  Yes
                <%end if%>
      ...
      <div class="row p-1">
          ...
          <div class="col-3">
              ...
              value="<% If tcWebApi_get("WLan_Entry","enrolleePinCode","h") = "N/A" then asp_Write("") else TCWebApi_get("WLan_Entry","enrolleePinCode","s") end if %>">
          </div>
          <div class="col-3 left">
