...
if Request_Form("btnApply_Flag") = "1" then
  TCWebApi_set("WLan_Entry","AccessPolicy","accessMode")
  TCWebApi_set("WLan_Entry","AccessMode","accessMode")

  if tcWebApi_get("WLan_Entry","AccessPolicy","h") = "1" then
	if Request_Form("W_macIndex_Flag") = "0" then
	  TCWebApi_set("WLan_Entry","W_WLan_MAC0","macAddr")
	elseif Request_Form("W_macIndex_Flag") = "1" then
	  TCWebApi_set("WLan_Entry","W_WLan_MAC1","macAddr")
	elseif Request_Form("W_macIndex_Flag") = "2" then
	  TCWebApi_set("WLan_Entry","W_WLan_MAC2","macAddr")
	elseif Request_Form("W_macIndex_Flag") = "3" then
	  TCWebApi_set("WLan_Entry","W_WLan_MAC3","macAddr")
	elseif Request_Form("W_macIndex_Flag") = "4" then
	  TCWebApi_set("WLan_Entry","W_WLan_MAC4","macAddr")
	elseif Request_Form("W_macIndex_Flag") = "5" then
	  TCWebApi_set("WLan_Entry","W_WLan_MAC5","macAddr")
	elseif Request_Form("W_macIndex_Flag") = "6" then
	  TCWebApi_set("WLan_Entry","W_WLan_MAC6","macAddr")
	elseif Request_Form("W_macIndex_Flag") = "7" then
	  TCWebApi_set("WLan_Entry","W_WLan_MAC7","macAddr")
	end if
  elseif tcWebApi_get("WLan_Entry","AccessPolicy","h") = "2" then  
	if Request_Form("B_macIndex_Flag") = "0" then
	  TCWebApi_set("WLan_Entry","B_WLan_MAC0","macAddr")
	elseif Request_Form("B_macIndex_Flag") = "1" then
	  TCWebApi_set("WLan_Entry","B_WLan_MAC1","macAddr")
	elseif Request_Form("B_macIndex_Flag") = "2" then
	  TCWebApi_set("WLan_Entry","B_WLan_MAC2","macAddr")
	elseif Request_Form("B_macIndex_Flag") = "3" then
	  TCWebApi_set("WLan_Entry","B_WLan_MAC3","macAddr")
	elseif Request_Form("B_macIndex_Flag") = "4" then
	  TCWebApi_set("WLan_Entry","B_WLan_MAC4","macAddr")
	elseif Request_Form("B_macIndex_Flag") = "5" then
	  TCWebApi_set("WLan_Entry","B_WLan_MAC5","macAddr")
	elseif Request_Form("B_macIndex_Flag") = "6" then
	  TCWebApi_set("WLan_Entry","B_WLan_MAC6","macAddr")
	elseif Request_Form("B_macIndex_Flag") = "7" then
	  TCWebApi_set("WLan_Entry","B_WLan_MAC7","macAddr")
	end if
  end if
...
if Request_Form("btnDel_Action") = "1" then
  if Request_Form("accessMode") = "1" then
    if Request_Form("btnDel_Flag0") = "1" then
      tcWebApi_constSet("WLan_Entry","W_WLan_MAC0","")
    end if
    if Request_Form("btnDel_Flag1") = "1" then
      tcWebApi_constSet("WLan_Entry","W_WLan_MAC1","")
    end if
    if Request_Form("btnDel_Flag2") = "1" then
      tcWebApi_constSet("WLan_Entry","W_WLan_MAC2","")
    end if
    if Request_Form("btnDel_Flag3") = "1" then
      tcWebApi_constSet("WLan_Entry","W_WLan_MAC3","")
    end if
    if Request_Form("btnDel_Flag4") = "1" then
      tcWebApi_constSet("WLan_Entry","W_WLan_MAC4","")
    end if
    if Request_Form("btnDel_Flag5") = "1" then
      tcWebApi_constSet("WLan_Entry","W_WLan_MAC5","")
    end if
    if Request_Form("btnDel_Flag6") = "1" then
      tcWebApi_constSet("WLan_Entry","W_WLan_MAC6","")
    end if
    if Request_Form("btnDel_Flag7") = "1" then
      tcWebApi_constSet("WLan_Entry","W_WLan_MAC7","")
    end if
  elseif Request_Form("accessMode") = "2" then
    if Request_Form("btnDel_Flag0") = "1" then
      tcWebApi_constSet("WLan_Entry","B_WLan_MAC0","")
    end if
    if Request_Form("btnDel_Flag1") = "1" then
      tcWebApi_constSet("WLan_Entry","B_WLan_MAC1","")
    end if
    if Request_Form("btnDel_Flag2") = "1" then
      tcWebApi_constSet("WLan_Entry","B_WLan_MAC2","")
    end if
    if Request_Form("btnDel_Flag3") = "1" then
      tcWebApi_constSet("WLan_Entry","B_WLan_MAC3","")
    end if
    if Request_Form("btnDel_Flag4") = "1" then
      tcWebApi_constSet("WLan_Entry","B_WLan_MAC4","")
    end if
    if Request_Form("btnDel_Flag5") = "1" then
      tcWebApi_constSet("WLan_Entry","B_WLan_MAC5","")
    end if
    if Request_Form("btnDel_Flag6") = "1" then
      tcWebApi_constSet("WLan_Entry","B_WLan_MAC6","")
    end if
    if Request_Form("btnDel_Flag7") = "1" then
      tcWebApi_constSet("WLan_Entry","B_WLan_MAC7","")
    end if
  end if
  TCWebApi_commit("WLan_Entry");
  TCWebApi_save();
...
if Request_Form("btnApply_Flag") = "0" then
...
<body onload="on_load()">
  ...
  <div class="container-full container-resize-height">
    <form name="ConfigForm" action="/cgi-bin/true-page/wlan/wlan-1-access.asp" method="post" >
    ...
    <div class="row p-1">
      ...
      <div class="col-6">
        <select class="form-control input-select" name="SSID_INDEX" SIZE="1" onChange="doSSIDChange()">
          ...
          <%if 1 <> 1 then%>
          <option value="5" <% if TCWebApi_get("WebCurSet_Entry","wlan_id","h") = "5" then asp_write("selected") end if %> >SSID5</option>
          <option value="6" <% if TCWebApi_get("WebCurSet_Entry","wlan_id","h") = "6" then asp_write("selected") end if %> >SSID6</option>
          <option value="7" <% if TCWebApi_get("WebCurSet_Entry","wlan_id","h") = "7" then asp_write("selected") end if %> >SSID7</option>
          <% end if %>
        </select>
      </div>
    </div>
    ...
    <div class="row p-1">
      ...
      <div class="col-6">
        <input TYPE="RADIO" id="btnRadio1" onclick="typeClick()" NAME="WLAN_FltActive" VALUE="1" <% If tcWebApi_get("WLan_Entry","AccessPolicy","h") <> "0" then asp_Write("checked") end if %> >
        <label class="input-radio">Enable</label>
        <input TYPE="RADIO" id="btnRadio2" onclick="typeClick()" NAME="WLAN_FltActive" VALUE="0" <% If tcWebApi_get("WLan_Entry","AccessPolicy","h") = "0" then asp_Write("checked") end if %> <% If tcWebApi_get("WLan_Entry","AccessPolicy","h") = "N/A" then asp_Write("checked") end if %> >
    ...
    <div id="cfg_list" class="container-resize-height element-hide" >
      ...
      <div class="row p-1">
        ...
        <div class="col-3">
          ...
          -->
          <input type="radio" value="1" name="accessMode" onclick="ChangeShowList(this.value)" <% if tcWebApi_get("WLan_Entry","AccessMode","h") = "1" then asp_Write("checked") end if %>>
          <label class="input-radio">Whitelist</label>
          <input type="radio" value="2" name="accessMode" onclick="ChangeShowList(this.value)" <% if tcWebApi_get("WLan_Entry","AccessMode","h") = "2" then asp_Write("checked") end if %>>
  ...
  <script language="JavaScript" type="text/JavaScript">
    var modeEn = '<% tcWebApi_get("WLan_Entry","AccessPolicy","s") %>';
    var enable_state;
    ...
    var W_macListObj = [
      {
        modeValue: "WhiteList",
        mac: '<% tcWebApi_get("WLan_Entry","W_WLan_MAC0","s") %>',
      },
      {
        modeValue: "WhiteList",
        mac: '<% tcWebApi_get("WLan_Entry","W_WLan_MAC1","s") %>',
      },
      {
        modeValue: "WhiteList",
        mac: '<% tcWebApi_get("WLan_Entry","W_WLan_MAC2","s") %>',
      },
      {
        modeValue: "WhiteList",
        mac: '<% tcWebApi_get("WLan_Entry","W_WLan_MAC3","s") %>',
      },
      {
        modeValue: "WhiteList",
        mac: '<% tcWebApi_get("WLan_Entry","W_WLan_MAC4","s") %>',
      },
      {
        modeValue: "WhiteList",
        mac: '<% tcWebApi_get("WLan_Entry","W_WLan_MAC5","s") %>',
      },
      {
        modeValue: "WhiteList",
        mac: '<% tcWebApi_get("WLan_Entry","W_WLan_MAC6","s") %>',
      },
      {
        modeValue: "WhiteList",
        mac: '<% tcWebApi_get("WLan_Entry","W_WLan_MAC7","s") %>',
      }
    ];
    ...
    var B_macListObj = [
      {
        modeValue: "BlackList",
        mac: '<% tcWebApi_get("WLan_Entry","B_WLan_MAC0","s") %>',
      },
      {
        modeValue: "BlackList",
        mac: '<% tcWebApi_get("WLan_Entry","B_WLan_MAC1","s") %>',
      },
      {
        modeValue: "BlackList",
        mac: '<% tcWebApi_get("WLan_Entry","B_WLan_MAC2","s") %>',
      },
      {
        modeValue: "BlackList",
        mac: '<% tcWebApi_get("WLan_Entry","B_WLan_MAC3","s") %>',
      },
      {
        modeValue: "BlackList",
        mac: '<% tcWebApi_get("WLan_Entry","B_WLan_MAC4","s") %>',
      },
      {
        modeValue: "BlackList",
        mac: '<% tcWebApi_get("WLan_Entry","B_WLan_MAC5","s") %>',
      },
      {
        modeValue: "BlackList",
        mac: '<% tcWebApi_get("WLan_Entry","B_WLan_MAC6","s") %>',
      },
      {
        modeValue: "BlackList",
        mac: '<% tcWebApi_get("WLan_Entry","B_WLan_MAC7","s") %>',
      }
    ];
  ...
  function on_load()
  {
    var show = '<% tcWebApi_get("WLan_Entry","AccessPolicy","s") %>';
