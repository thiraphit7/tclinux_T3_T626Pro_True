...
If request_Form("VoIP_AppBtn_flag")="1" then
	...
	if tcWebApi_get("WebCurSet_Entry", "VoIPIdle", "h") = "1"  then
		...
		tcWebApi_constSet("VoIPBasic_Entry0", "Enable", "Yes")	
		...
		'tcWebApi_set("VoIPCallCtrl_Entry0", "conference-uri", "conf-uri")	
...
<head>
  ...
  <SCRIPT language=JavaScript type="text/javascript">
	...
	function lineStatus()
	{
		...
		<%if tcWebAPI_get("VoIPBasic_Entry0", "Enable", "h") = "Yes" then %>
			form.authName0.disabled = false;
			form.password0.disabled = false;
			form.extension0.disabled = false;
			form.dispName0.disabled = false;
		<% else %>
			form.authName0.disabled = true;
			form.password0.disabled = true;
			form.extension0.disabled = true;
			form.dispName0.disabled = true;
		<% end if %>	
...
<body onload="on_load()">
  ...
  <div class="container-full container-resize-height">
   <FORM id=ConfigForm name="ConfigForm" action="/cgi-bin/true-page/vo-ip/port2.asp" method="post">
		...
		<% end if%>
<% if tcwebApi_get("VoIPBasic_Common","VoIPLine2Enable","h") = "Yes" then %>
        <div class="title-text">Port2 Configuration</div>
        <div class="title-description-text">This page is used to configure Port2.</div>
<% else %>
...
<% end if %>
      ...
      <br>
<% if tcwebApi_get("VoIPBasic_Common","VoIPLine2Enable","h") = "Yes" then %>
      <div class="title-text">Pots2</div>
<% else %>
...
<% end if %>
      <div class="row p-1" id="edit_telnumber">
          ...
          <div class="col-6">
              <input class="form-control input-textfield" type="text" name='extension0' size=20 maxlength=64 value="<% tcWebApi_get("VoIPBasic_entry0","SIPDisplayName","s") %>">
          </div>
      </div>
      <div class="row p-1" id="edit_displayname" >
          ...
          <div class="col-6">
              <input class="form-control input-textfield" type="text" size=20 maxlength=128 name='dispName0' value="<% tcWebApi_get("VoIPBasic_entry0","SIPDisplayNumber","s") %>">
          </div>
      </div>
      <div class="row p-1">
          <div class="col-6">
          ...
          <div class="col-6">
			  <input class="form-control input-textfield" type='text' size=20 maxlength=128 name='authName0'  value='<%if tcWebAPI_get("VoIPBasic_Entry0", "SIPAuthenticationName", "h") <> "N/A" then tcWebAPI_get("VoIPBasic_Entry0", "SIPAuthenticationName", "s") else asp_Write("") end if %>'>
          </div>
      </div>
      <div class="row p-1">
          <div class="col-6">
          ...
          <div class="col-6">
			  <input class="form-control input-textfield" type='Password' size=20 maxlength=128 name='password0' value='<%if tcWebAPI_get("VoIPBasic_Entry0", "SIPPassword", "h") <> "N/A" then tcWebAPI_get("VoIPBasic_Entry0", "SIPPassword", "s") else asp_Write("") end if %>'>
          </div>
      </div>
      <div class="row p-1">
          <div class="col-6">
      ...
      <div class="row p-1">
          ...
          <div class="col-6">
              <input type="checkbox" value="1" name="callwait0" <%if tcWebApi_get("VoIPCallCtrl_Entry0", "SIPCallWaitingEnable", "h") = "1" then asp_Write("checked") end if %>>
              <label class="input-radio">Enable</label>
          </div>
      </div>	 
      <div class="row p-1">
          ...
          <div class="col-6">
              <input type="checkbox" name="transfer0" <%if tcWebApi_get("VoIPCallCtrl_Entry0", "SIPCallTransfer", "h") = "Yes" then asp_Write("checked") end if %>>
              <label class="input-radio">Enable</label>
          </div>
      </div>
      <div class="row p-1">
          ...
          <div class="col-6">
              <input type="checkbox" name="threepartyservice0" <%if tcWebApi_get("VoIPCallCtrl_Entry0", "SIP3wayConf", "h") = "Yes" then asp_Write("checked") end if %>>
              <label class="input-radio">Enable</label>
          </div>
      </div>
      <!--
      <div class="row p-1">
          ...
          <div class="col-6">
              <input class="form-control input-textfield" name="conf-uri" type="text" value="<% tcWebApi_get("VoIPCallCtrl_Entry0","conference-uri","s")%>" >
          </div>
      </div>
      -->
      <br>
  ...
  <script>
	...
	function btnApplySip() 
	{
	   ...
	   {					
			...
			if (authName0.value == "")
			{
				alert('SIP Account can not be empty!');
				return;
			}
			if (authName0.value.indexOf("\"") >= 0)
			{
				alert('Invalid SIP Account!');
				return;
			}
			
			if (password0.value != "" && password0.value.indexOf("\"") >= 0)
			{
...
<% end if %>
	...
	<% end if %>
			...
			if (getElById("callwait0").checked)
			{
				callwait0Flag.value = "1";
			}
			
			if (getElById("transfer0").checked)
			{
				transfer0Flag.value = "Yes";
			}
			
			if (getElById("threepartyservice0").checked)
			{
