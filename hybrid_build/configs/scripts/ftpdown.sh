...
then
	...
	then	#user save path is not "", create the fold then download
		...
		if [ $? -eq 0 ]
		then
			/usr/bin/ftpget -u "${4}" -p "${5}" -P "${3}" "${2}" "${rootpath}/${7}/${8}/${6##*/}" "${6}" 2>/tmp/ftpdwload.log
			local result="`cat /tmp/ftpdwload.log`"
			ret=`check_download "$result"`
			rm -f /tmp/ftpdwload.log
		else
			ret=2   #create fold errorid
		fi
	else	##user save path is "", direct download
		/usr/bin/ftpget -u "${4}" -p "${5}" -P "${3}" "${2}" "${rootpath}/${7}/${6##*/}" "${6}" 2>/tmp/ftpdwload.log
		local result="`cat /tmp/ftpdwload.log`"
		ret=`check_download "$result"`
		rm -f /tmp/ftpdwload.log
	fi
